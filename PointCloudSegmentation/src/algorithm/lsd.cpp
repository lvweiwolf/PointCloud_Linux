...
#define TABSIZE 100000
										...
										double density_th)
		{
			...
			while (density < density_th)
			{
				...
				density =
					(double)*reg_size / (dist(rec->x1, rec->y1, rec->x2, rec->y2) * rec->width);
			}
						  ...
						  double density_th)
		{
			...
			if (density < density_th)
				return reduce_region_radius(reg,
											reg_size,
											modgrad,
											reg_angle,
											prec,
									 ...
									 int* reg_y)
		{
			...
			if (scale != 1.0)
			{
				...
				angles =
					ll_angle(scaled_image, rho, &list_p, &mem_p, &modgrad, (unsigned int)n_bins);
