	...
	namespace pcs {
		namespace gdal {
			...
			namespace {
				...
				*/
				Type toPdalType(const std::string& gdalType)
				{
					if (gdalType == "Byte")
				...
				Type toPdalType(GDALDataType t)
				{
					switch (t)
					{
					case GDT_Byte:
					...
					case GDT_CFloat32:
					case GDT_CFloat64:
						throw std::runtime_error("GDAL complex float type unsupported.");
					case GDT_Unknown:
						throw std::runtime_error("GDAL unknown type unsupported.");
					case GDT_TypeCount:
						throw std::runtime_error("Detected bad GDAL data type.");
					case GDT_Int8:
						return Type::Signed8;
					case GDT_UInt64:
				...
				GDALDataType toGdalType(Type t)
				{
					switch (t)
					{
					case Type::Unsigned8:
					...
					case Type::Signed64:
						throw std::runtime_error("PDAL 64-bit integer type unsupported.");
					case Type::None:
						throw std::runtime_error("PDAL 'none' type unsupported.");
					default:
						throw std::runtime_error("Unrecognized PDAL dimension type.");
					}
				}
			...
			*/
			GDALError Raster::open(int width,
								   int height,
								   int numBands,
								   Type type,
								   double noData,
								   std::vector<std::string> options)
			{
				...
				if (!GDALInvGeoTransform(m_forwardTransform.data(), m_inverseTransform.data()))
				{
					m_errorMsg = "Geotransform for raster '" + m_filename +
								 "' not "
								 "invertible";
				...
				GDALDriver* driver = GetGDALDriverManager()->GetDriverByName(m_drivername.data());
				...
				if (item != "YES")
				{
					m_errorMsg = "Requested driver '" + m_drivername +
								 "' does not "
				...
				m_ds = driver->Create(m_filename.data(),
									  m_width,
									  m_height,
									  m_numBands,
									  toGdalType(type),
									  const_cast<char**>(opts.data()));
			...
			*/
			GDALError Raster::wake()
			{
				...
				{
					m_errorMsg = "Unable to get geotransform for raster '" + m_filename + "'.";
			...
			/**
			  ...
			  \return  Requested type, or if not supported, the preferred type to use
				  for the raster.
			*/
			GDALError Raster::validateType(Type& type, GDALDriver* driver)
			{
				...
				if (itemp)
				{
					std::vector<std::string> items = StringSplit(std::string(itemp), " ");
					for (auto& i : items)
					{
						Type t = toPdalType(i);
						if (t != Type::None)
				...
				{
					m_errorMsg = "Requested type '" + interpretationName(type) +
			...
			*/
			bool Raster::getPixelAndLinePosition(double x, double y, int32_t& pixel, int32_t& line)
			{
				pixel = (int32_t)std::floor(m_inverseTransform[0] + (m_inverseTransform[1] * x) +
											(m_inverseTransform[2] * y));
				line = (int32_t)std::floor(m_inverseTransform[3] + (m_inverseTransform[4] * x) +
										   (m_inverseTransform[5] * y));

				// Return false if we're out of bounds.
				return (pixel >= 0 && pixel < m_width && line >= 0 && line < m_height);
			}
			...
			GDALError Raster::computePDALDimensionTypes()
			{
				...
				for (int i = 0; i < m_numBands; ++i)
				{
					...
					if (!band)
					{
						...
						oss << "Unable to get band " << (i + 1) << " from raster data source '"
							<< m_filename << "'.";
						m_errorMsg = oss.str();
			...
			*/
			std::string Raster::getSpatialRef() const
			{
				std::string srs;

				if (m_ds)
					srs = m_ds->GetProjectionRef();
			...
			*/
			...
			GDALError Raster::statistics(int nBand,
										 double* minimum,
										 double* maximum,
										 double* mean,
										 double* stddev,
										 ...
										 bool force) const
			{
				try
				{
					Band<double>(m_ds, nBand)
						.statistics(minimum, maximum, mean, stddev, approx, force);
				}
				catch (InvalidBand)
				{
					m_errorMsg = "Unable to get band " + std::to_string(nBand) + " from raster '" +
								 m_filename + "'.";
					return GDALError::InvalidBand;
				}
				catch (BadBand)
