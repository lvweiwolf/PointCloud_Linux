//////////////////////////////////////////////////////////////////////
// 文件名称：icp.h
// 功能描述：Iterative Closest Points 点云配准算法
// 创建标识：吕伟	2022/12/16
// 修改标识：
// 修改描述：
// 文件版权：江西博微新技术有限公司
...
#include <src/algorithm/anderson_acceleration.h>
...
/*
	KD树适配器，用于处理直接存储在特征矩阵中的数据，而无需复制数据存储。
	此代码改编自nanoflann.hpp的KDTreeEigenMatrixAdaptor类
*/
namespace nanoflann {
	...
	struct KDTreeAdaptor
	{
		...
		KDTreeAdaptor(const MatrixType& mat, const int leaf_max_size = 10) : m_data_matrix(mat)
		{
			...
			index =
				new index_t(dims, *this, nanoflann::KDTreeSingleIndexAdaptorParams(leaf_max_size));
			index->buildIndex();
		}
...
namespace d3s {
	namespace pcs {
		...
		// point-to-point 和 point-to-plane 距离刚性变换
...
#pragma omp section
		...
		namespace SICP {
			...
			{
				bool use_penalty =
					false; /// if use_penalty then penalty method else ADMM or ALM (see max_inner)
				double p = 1.0;			 /// p norm
			...
			{
				return shrinkage<I - 1>(mu,
										n,
										p,
										1.0 -
											(p / mu) * std::pow(n, p - 2.0) * std::pow(s, p - 1.0));
			}
			template <>
...
#pragma omp parallel for
			...
			void point_to_point(Eigen::MatrixBase<Derived1>& X,
								...
								Eigen::Vector3d& target_mean,
								Parameters& par)
			{
				/// Build kd-tree
				nanoflann::
					KDTreeAdaptor<Eigen::MatrixBase<Derived2>, 3, nanoflann::metric_L2_Simple>
						kdtree(Y);
				/// Buffers
...
#pragma omp parallel for
				...
				if (par.print_icpn)
				{
					...
					{
						PCS_DEBUG("[SICP::point_to_point] %lf %lf", times[i], gt_mses[i]);
					}
				}
...
#pragma omp parallel for
					...
					if (par.print_icpn)
					{
						PCS_DEBUG("[SICP::point_to_plane] iter = %d, time = %lf, gt_mse = %lf",
								  icp,
								  run_time,
								  gt_mse);
					}
					...
					for (int outer = 0; outer < par.max_outer; ++outer)
					{
						...
						/// C update (lagrange multipliers)
						Eigen::VectorXd P =
							(Qn.array() * (X - Qp).array()).colwise().sum().transpose() - Z.array();
						if (!par.use_penalty)
							C.noalias() += mu * P;
						/// mu update (penalty)
						if (mu < par.max_mu)
				...
				{
					for (int i = 0; i < (int)times.size(); i++)
						PCS_DEBUG("[SICP::point_to_plane] %lf %lf", times[i], gt_mses[i]);
				}
		...
		namespace ICP {
			...
			/// @param Residuals
			/// @param Parameter
			inline void uniform_weight(Eigen::VectorXd& r) { r = Eigen::VectorXd::Ones(r.rows()); }
			/// @param Residuals
			/// @param Parameter
			...
			inline void autowelsch_weight(Eigen::VectorXd& r, double p)
			{
				double mid;
				median(r, mid);
				welsch_weight(r, p * mid / (std::sqrt(2) * 2.3));
				// welsch_weight(r,p);
			}
		...
		namespace AAICP {
			...
			///////////////////////////////////////////////////////////////////////////////////////////
			inline VectorX get_next_u(const Matrix6X u,
							   const Matrix6X g,
							   const Matrix6X f,
...
#pragma omp parallel for
					...
					if (par.print_energy)
					{
						PCS_DEBUG("[AAICP::point_to_point_aaicp] icp iter = %d, Energy = %lf, "
								  "gt_mse = %lf",
								  icp,
								  energy,
								  gt_mse);
				...
				if (par.print_output)
				{
					for (int i = 0; i < icp; i++)
					{
						PCS_DEBUG("[AAICP::point_to_point_aaicp] %lf %lf %lf",
								  times[i],
								  energys[i],
								  gt_mses[i]);
					}
