...
// 文件版权：江西博微新技术有限公司
//////////////////////////////////////////////////////////////////////
#ifndef ICP_FAST_ROBUST_H_
#define ICP_FAST_ROBUST_H_

#include <src/algorithm/icp.h>
...
namespace d3s {
	namespace pcs {
		...
		private:
			...
			{
				AffineMatrixN m = AffineMatrixN::Zero();
				m << 0, -v[2], v[1], v[3], v[2], 0, -v[0], v[4], -v[1], v[0], 0, v[5], 0, 0, 0, 0;
				return m;
			}
...
#pragma omp parallel for
				for (int i = 0; i < X.cols(); i++)
				{
					...
					median(k_dist.tail(nk - 1), X_nearest[i]);
					delete[] id;
					delete[] dist;
				}
				double med;
				median(X_nearest, med);
				return sqrt(med);
			}
			/// Find self normal edge median of point cloud
			double FindKnearestNormMed(const KDtree& kdtree,
									   const Eigen::Matrix3Xd& X,
									   int nk,
									   const Eigen::Matrix3Xd& norm_x)
			{
				Eigen::VectorXd X_nearest(X.cols());
#pragma omp parallel for
				for (int i = 0; i < X.cols(); i++)
											  ...
											  Vector6& dir)
			{
				...
				if (theta > SAME_THRESHOLD)
				{
					coeff << st / theta, (1 - ct) / (theta * theta),
						...
						(theta * st - 2 * (1 - ct)) / pow(theta, 4), (1 - ct) / (theta * theta),
						(theta - st) / pow(theta, 3), (theta * st - 2 * (1 - ct)) / pow(theta, 4),
				...
				Matrix93 dRdw = coeff(0, 0) * dbdw + coeff(1, 0) * tempB3 + coeff(2, 0) * db2dw +
								coeff(3, 0) * temp2B3;
...
#pragma omp for
		...
		public:
			void point_to_point(MatrixNX& X,
								MatrixNX& Y,
								VectorN& source_mean,
								VectorN& target_mean,
								ICP::Parameters& par)
...
#pragma omp parallel for
						...
						if (par.print_energy)
						{
							PCS_DEBUG("[FRICP::point_to_point] icp iter = %d, Energy = %lf, "
									  "time = %lf",
									  icp,
									  energy,
									  run_time);
...
#pragma omp parallel for
				...
				if (par.print_output)
				{
					...
					{
						PCS_DEBUG("[FRICP::point_to_point] %lf %lf %lf",
								  times[i],
								  energys[i],
								  gt_mses[i]);
					}
			...
			void point_to_plane(Eigen::Matrix3Xd& X,
								Eigen::Matrix3Xd& Y,
								Eigen::Matrix3Xd& norm_x,
								Eigen::Matrix3Xd& norm_y,
...
#pragma omp parallel for
						...
						if (par.print_energy)
						{
							PCS_DEBUG("[FRICP::point_to_plane] icp iter = %d, Energy = %lf, "
									  "gt_mse = %lf",
									  total_iter,
									  energy,
									  gt_mse);
				...
				if (par.print_output)
				{
					...
					{
						PCS_DEBUG("[FRICP::point_to_plane] %lf %lf %lf",
								  times[i],
								  energys[i],
								  gt_mses[i]);
					}
...
#pragma omp parallel for
						...
						if (par.print_energy)
						{
							PCS_DEBUG("[FRICP::point_to_plane_GN] icp iter = %d, gt_mse = %lf, nu1 "
									  "= %lf, acept_aa = %d,  energy = %lf",
									  total_iter,
									  gt_mse,
									  nu1,
									  n_linsearch,
...
#pragma omp parallel for
				...
				if (par.print_output)
				{
					...
					for (int i = 0; i < total_iter; i++)
					{
						PCS_DEBUG("[FRICP::point_to_plane_GN] %lf %lf %lf",
								  times[i],
								  energys[i],
								  gt_mses[i]);
					}
