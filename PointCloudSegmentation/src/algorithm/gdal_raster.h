...
// 文件版权：江西博微新技术有限公司
//////////////////////////////////////////////////////////////////////
#ifndef GDAL_RASTER_H_
#define GDAL_RASTER_H_
		...
		namespace gdal {
			...
			*/
			...
			protected:
				...
				void statistics(double* minimum,
								double* maximum,
								double* mean,
								double* stddev,
								bool approx,
			...
			private:
				...
				*/
				Band(GDALDataset* ds,
					 int bandNum,
					 double dstNoData = -9999.0,
					 const std::string& name = "")
					: BaseBand(ds, bandNum, name),
					  m_dstNoData(dstNoData),
					  m_xBlockSize(0),
					  m_yBlockSize(0)
				{
					int xTotalSize;
					...
					if (xBlockSize <= 0 || yBlockSize <= 0 || xTotalSize <= 0 || yTotalSize <= 0)
						throw BadBand();
				...
				*/
				...
				void readBlock(size_t x, size_t y, std::vector<T>& data)
				{
					...
					readBlockBuf(static_cast<int>(x),
								 static_cast<int>(y),
				...
				*/
				...
				T getNoData() const
				{
					...
					if (!numericCast(m_dstNoData, t))
					{
						throw CantWriteBlock(
							StringPrintf("Invalid nodata value %.lf or output data_type '%s'.",
										 m_dstNoData,
										 typeid(T).name()));
					}
								...
								ITER_VAL<SOURCE_ITER> srcNoData)
				{
					...
					for (size_t row = 0; row < yHeight; ++row)
					{
						...
						std::transform(
							...
							[srcNoData, dstNoData](ITER_VAL<SOURCE_ITER> s) {
								...
								if (srcNoData == s || (std::isnan(srcNoData) && std::isnan(s)))
									t = dstNoData;
								else
								{
									if (!numericCast(s, t))
					...
					writeBlockBuf(static_cast<int>(x),
								  static_cast<int>(y),
			...
			public:
				...
				*/
				...
				GDALError readBand(std::vector<T>& points, int nBand)
				{
					...
					{
						m_errorMsg = "Unable to read block for for raster '" + m_filename + "'.";
				...
				*/
				template <typename SOURCE_ITER>
				GDALError writeBand(SOURCE_ITER si,
									ITER_VAL<SOURCE_ITER> srcNoData,
									int nBand,
									const std::string& name = "")
				{
					try
					{
						...
						{
						case Type::Unsigned8:
							Band<uint8_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Signed8:
							Band<int8_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Unsigned16:
							Band<uint16_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Signed16:
							Band<int16_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Unsigned32:
							Band<uint32_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Signed32:
							Band<int32_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Unsigned64:
							Band<uint64_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Signed64:
							Band<int64_t>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Float:
							Band<float>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::Double:
							Band<double>(m_ds, nBand, m_dstNoData, name).write(si, srcNoData);
							break;
						case Type::None:
							throw CantWriteBlock();
						}
					...
					{
						m_errorMsg = "Unable to write block for for raster '" + m_filename + "'.";
						if (err.what.size())
							m_errorMsg += "\n" + err.what;
				...
				*/
				std::vector<Type> getPDALDimensionTypes() const { return m_types; }
				...
				*/
				int bandCount() const { return m_numBands; }
				...
				*/
				...
				GDALError statistics(int nBand,
									 double* minimum,
									 double* maximum,
									 double* mean,
									 double* stddev,
			...
			private:
				GDALError validateType(Type& type, GDALDriver* driver);

				bool getPixelAndLinePosition(double x, double y, int32_t& pixel, int32_t& line);
				...
				mutable std::vector<Type> m_types;
