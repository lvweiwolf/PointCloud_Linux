...
#include "../core/deeplearning/inference.h"
...
namespace d3s {
	namespace pcs {
		...
		protected:
			...
			* @return
			*/
			std::vector<std::vector<osg::Vec3d>> Pixel2Real(const d3s::pcs::BoundingBox2D& regionBox, const std::vector<std::vector<cv::Point>>& regionPts
			...
			* @return
			*/
			void MergeIntersectPolygon(std::vector<std::vector<osg::Vec3d>>& polygons, double dEpsilon);
			...
			* @return
			*/
			bool IntersectsPolygon(const std::vector<osg::Vec3d>& polygon1, const std::vector<osg::Vec3d>& polygon2);
