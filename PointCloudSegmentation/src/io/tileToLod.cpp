...
namespace d3s {
	namespace pcs {
		namespace io {
			...
			AxisInfo TileToLOD::FindMaxAxis(osg::BoundingBox boundingBox,
			...
			bool TileToLOD::Generate(const std::vector<PointRGBI>* pointSet,
									 ...
									 osg::BoundingBox& boundingBoxLevel0)
			{
				...
				bool build = BuildNode(pointSet,
									   ...
									   0,
									   exportMode);
				std::vector<unsigned int> tempPointIndex;
				pointIndex.swap(tempPointIndex);
			...
			osg::Geode* TileToLOD::MakeNodeGeode(const std::vector<PointRGBI>* pointSet,
												 std::vector<unsigned int>& pointIndex,
												 ExportMode exportMode)
			{
				...
				for (std::vector<unsigned int>::iterator i = pointIndex.begin();
					 i != pointIndex.end();
					 i++)
				{
					PointRGBI tmpPoint = pointSet->at(*i);
					...
					{
						colorArray->push_back(osg::Vec4(Color8BitsToFloat(tmpPoint.C[0]),
					...
					{
						float x =
							(tmpPoint.P.z() - this->_boundingBoxGlobal.zMin()) /
							(this->_boundingBoxGlobal.zMax() - this->_boundingBoxGlobal.zMin());
						//// sigmoid
						// x = (x - 0.5) * 4;
						// x = 1. / (1. + exp(-5 * x));
						int index = x * 255;
				...
				geometry->addPrimitiveSet(
					new osg::DrawArrays(osg::PrimitiveSet::POINTS, 0, pointIndex.size()));
			...
			bool TileToLOD::BuildNode(const std::vector<PointRGBI>* pointSet,
									  std::vector<unsigned int>& pointIndex,
									  osg::BoundingBox boundingBox,
									  osg::BoundingBox boundingBoxLevel0,
									  const std::string& saveFilePath,
									  ...
									  ExportMode exportMode)
			{
				...
				{
					PCS_ERROR("[pcl::io::TileToLOD::BuildNode] Mode %d is NOT support!.",
							  exportMode);
					return false;
				}
				...
				if (pointIndex.size() < _maxPointNumPerOneNode || level >= _maxTreeLevel)
				{
					osg::ref_ptr<osg::Geode> nodeGeode =
						MakeNodeGeode(pointSet, pointIndex, exportMode);
					...
					{
						/*if (!ConvertOsgbTo3mxb(nodeGeode, saveFileName))
						{
							seed::log::DumpLog(seed::log::Critical,
											   "Write node file %s failed!",
											   saveFileName.c_str());
							return false;
						}*/
						PCS_ERROR("[pcl::io::TileToLOD::BuildNode] Mode %d is NOT support!.",
				...
				for (int i = 0; i < pointIndex.size(); i++)
				{
					...
					{
						PointRGBI tmpPoint = pointSet->at(pointIndex[i]);
				...
				// export
				{
					...
					osg::ref_ptr<osg::Geode> nodeGeode =
						MakeNodeGeode(pointSet, selfPointSetIndex, exportMode);
					mt->addChild(nodeGeode.get());
					std::vector<unsigned int> tempPointSetIndex;
					selfPointSetIndex.swap(tempPointSetIndex);
				...
				if (leftPointSetIndex.size())
				{
					...
					leftPointSetIndex.swap(tempIndex);
				}
				// recursive right
				if (rightPointSetIndex.size())
				{
					BuildNode(pointSet,
							  ...
							  exportMode);
					std::vector<unsigned int> tempIndex;
					rightPointSetIndex.swap(tempIndex);
				}
				return true;
			}
		}
