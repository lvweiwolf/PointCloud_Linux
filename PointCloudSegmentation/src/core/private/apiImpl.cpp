...
namespace d3s {
	namespace pcs {
		...
		bool readPointCloud2(const std::string& filename,
							 PointCloudView<PointPCLH>& pcv)
		{
...
#pragma omp parallel for
				...
				for (int i = 0; i < (int)indices.size(); ++i) 
				{
					...
					// 执行最近K邻域搜索
					if (kdtree->nearestKSearch(indices[i],
												  meanK + 1,
												  nn_indices,
												  nn_dists) == 0)
					{
...
#pragma omp parallel for
			...
			for (int i = 0; i < (int)pcv.size(); ++i)
			{
				...
				pcv[i].hag = heigtAboveGround;
				CHECK_MSG(!std::isnan(pcv[i].hag), StringPrintf("z=%lf zGround=%lf", z, zGround));
