...
namespace {
	...
	static bool isWow64()
	{
		...
		return aFunIsWow64 != NULL && aFunIsWow64(GetCurrentProcess(), &bIsWow64) &&
			   bIsWow64 != false;
	}
	...
	static uint32_t readCpuMask(const char* thePath)
	{
		...
		for (const char* aCharIter = aBuffer; aCharIter < anEnd && *aCharIter != '\n';)
		{
			...
			// if we're not at the end of the item, expect a dash and and integer; extract end
			// value.
...
//=======================================================================
int CTBBParallel::NbLogicalProcessors()
{
...
void CTBBParallel::forEach(UniversalIterator& theBegin,
