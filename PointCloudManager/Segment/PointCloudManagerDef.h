...
namespace pc
{
	...
	namespace data
	{
		...
		struct tagPagedLodFile
		{
			...
			friend bool operator<(const tagPagedLodFile& object1, const tagPagedLodFile& object2)
			{
				return object1.strPointInfoFile.Compare(object2.strPointInfoFile) +
					object1.strPointTexFile.Compare(object2.strPointTexFile) < 0;
			}
		};
		...
		{
			PointCloudBoundBox2D()
				: _dXmin(0.0), _dXmax(0.0), _dYmin(0.0), _dYmax(0.0)
			{}
			...
			{
				if (_dXmax <= rhs._dXmin || _dXmin >= rhs._dXmax
					|| _dYmax <= rhs._dYmin || _dYmin >= rhs._dYmax)
					return false;
			...
			*/
			...
			{
				if (_dXmax >= rhs._dXmax && _dXmin <= rhs._dXmin && _dYmax >= rhs._dYmax && _dYmin <= rhs._dYmin)
					return true;
		...
		struct BoundBoxCompare : public std::binary_function<PointCloudBoundBox2D, PointCloudBoundBox2D, bool>
		{
		...
		typedef std::map<PointCloudBoundBox2D, int, pc::data::BoundBoxCompare> PointCloudBoundToPointNum;
		...
		struct SegmentParam
		{
			...
			SegmentParam(CString strVolLevel, CString strTopographicFeatures, osg::Matrix matrix, const std::vector<osg::Vec3d>& vecTowerPos,
				const std::vector<osg::Vec3d>& vecSelectPoints, const std::map<char, bool>& showTypeMap)
				:_strVolLevel(strVolLevel), _strTopographicFeatures(strTopographicFeatures), _vpwMatrix(matrix), _vecTowerPos(vecTowerPos)
				, _vecSelectPoints(vecSelectPoints), _showTypeMap(showTypeMap), _epsg(-1), _nDenoiseLevel(0)
			{
			}
		};
		...
		{
			tagTravelInfo() :fRadius(0.0), bOnlyXYRadius(false), bOnlyInLine(false), eMode(ETravelMode::eFastWideTravel) {}
			osg::Vec3 beginPnt;
			osg::Vec3 endPnt;
			float fRadius;		// 搜索半径
		...
		struct SPolygonParam
		{
			...
			SPolygonParam(const pc::data::SPolygonParam& param) :_matVPW(param._matVPW), _polygonPoints(param._polygonPoints)
				, _sliceMatVPW(param._sliceMatVPW), _slicePolygonPoints(param._slicePolygonPoints), _bAllTraversal(param._bAllTraversal) {}
		...
		{
			SLightningParam() :_nPiece(0), _dLineWidth(0.0), _dBeginAngle(0.0), _dEndAngle(0.0)
				, _dSectionWidth(0.0), _nWireType(0), _nGWireType(0), _nGroundType(0), _nJumpType(0), _nWaterSysType(0) {}
		...
		struct treeInfo
		{
			...
			/*
				...
				》树顶到 系数M(0<M<1) 处点个数大于 系数N(0<N<1) 倍总点数，则H等于包围盒对角长度。反之等于PQ距离
			*/
		};
		//const CString gl_strTreeInfoCoefficientCfg = L"..\\PluginsConfig\\PointCloud\\树木信息系数.xml";
		...
		struct SClusterAfterClassify
		{
			...
			std::map<int, std::vector<std::vector<osg::Vec3d>>> polygonPerCluster; // 类簇的多边形轮廓
		...
		{
			SQMClassifyParam() :_nSegmentIndex(0), _bCoverCluster(false)
				, _bNeedCreateCluster(false), _bOnlyOneCluster(false), _forceWaitTask(false)
			...
			bool _bOnlyOneCluster; // 分类类型创建的簇是否需要唯一（需要唯一当存在改类型的簇时，后续创建仅追加）
		...
		{
			SVegetationAutoClassifyParam() :_dShrubMinHeight(0.0), _dShrubMaxHeight(0.0), _nVegetationType(0)
				, _nGrassType(0), _nShrubType(0), _nForestType(0), _nGoundType(0) {}
			bool IsValid() const
			{
				return (_towerPnts.size() == _angleBisectionDis.size() && _towerPnts.size() >= 2);
			}
