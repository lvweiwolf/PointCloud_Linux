	...
	BoundToFile &boundBoxToFiles)
{
	...
	for (size_t i = 0; i < nCnt; ++i)
	{
		...
		pc::data::PointCloudBoundBox2D boundBox(boundingBox.xMin(), boundingBox.xMax(), boundingBox.yMin(), boundingBox.yMax());
...
void CPointCloudBoxQuery::MtSetPointsCloudToPoints(SMtSetParam& param)
{
	...
	if(param.nDenoiseLevel > 0 && param.nDenoiseLevel <= denoiseLevels.params.size())
	{
		...
		param.pPointCloud->Denoise(denoiseParam._dRadius, denoiseParam._nMinPtSize, inliers, outliers);
