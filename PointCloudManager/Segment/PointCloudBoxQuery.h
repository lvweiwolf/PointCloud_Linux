...
#pragma once
#include "../stdafx.h"
#include "PointCloudManagerDefine.h"
#include "PointCloudManagerDef.h"

// 包围盒对应的点云节点列表
typedef std::map<pc::data::PointCloudBoundBox2D, std::vector<pc::data::CModelNodePtr>, pc::data::BoundBoxCompare> BoundToPointCloudPagedLodDic;

typedef std::map<pc::data::PointCloudBoundBox2D, std::vector<pc::data::tagPagedLodFile>, pc::data::BoundBoxCompare> BoundToFile;	// 包围盒区域对应的文件列表
...
public:
	...
	*  输入参数    	int nLevel													指定层级(-1为叶子级节点)
	*  输出参数    	std::vector<CPointCloudpagedLod> &pointCloudPagedLodList	指定层级的pagedlod列表
	*  输出参数	
	*  返回值   	void
	*/
	static const int nAllLevel = -1;	//获取所有层级pagedlod子节点
	static void GetLevelPagedLodList(
	...
	*  输入参数    	int nLevel													指定层级(-1为叶子级节点)
	*  输入参数    	std::vector<pc::data::CModelNodePtr> & pointCloudPagedLodList	指定层级的pagedlod列表
	*  输出参数	
	*  返回值   	void
	*/
	...
	*  输出参数    	std::vector<CPointCloudpagedLod> & pointCloudPagedLodList		叶子节点pagedLod
	*  输出参数
	*  返回值   	void
	*/
	static void GetLeafPagedLodList(
		pc::data::CModelNodePtr pPagedLod,
		std::vector<pc::data::CModelNodePtr> &pointCloudPagedLodList);
	...
	*  函数介绍    	初始化包围盒对应的文件列表
	*
	*  输入参数    	const std::vector<pc::data::CModelNodePtr> & pointCloudPagedLodList	pagedLod列表
	*  输出参数    	BoundToFile & boundBoxToFiles													包围盒对应的文件列表
	*  输出参数
	*  返回值   	void
	*/
	...
	*/
	...
	struct SMtSetParam
	{
		SMtSetParam(std::map<unsigned, osg::BoundingBox>& boxMap)
			:nDenoiseLevel(0), eSegment(ESegmentType::eAutoSegment), strPrjId(nullptr), clusterBoxMap(boxMap) {}
